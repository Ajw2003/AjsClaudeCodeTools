...
|---|---|---|---|
| `SessionStart` | `inject` | every session | Prints `rules/house-rules.md` **and** the machine profile into context as `additionalContext`. This *is* the CLAUDE.md replacement. On a remote session (`CLAUDE_CODE_REMOTE` set) it also appends a distinct block for `rules/handover-target.md` — the human's own machine, as opposed to the sandbox `hook.py` runs on — recorded content when present, or an instruction to find out and record it when not; a local session adds nothing here. |
