...
Move a branch onto a new base without losing commits only that branch held — the safe form of
...
Refuses while the branch holds work the base does not, and cherry-picks that work onto the new base
instead. Comparison is by **patch-id**, so a commit already merged via squash or rebase is seen as
present rather than reported as about-to-be-lost. Never deletes anything; prints the old tip so
`git reset --hard <sha>` restores it. A merged pull request makes a branch look finished, which is
exactly when a commit pushed after that merge gets discarded silently.
...
| `PreToolUse` | `guard` | `Bash`/`PowerShell` calls | Extracts the `command` field and textually matches it against rule patterns (hidden/background work, git history/index/remote writes, destructive deletes and discards, and **ref moves** — `checkout -B`, `switch -C`, `branch -D`, force-push — which orphan commits with no dirty working tree to warn anyone). The ref-move patterns are the one place matching is case-**sensitive** (via `(?-i:...)`), because the case is the meaning: `-b` creates a branch and `-B` resets one, `-d` refuses on unmerged commits and `-D` forces. A match returns `permissionDecision: "ask"` — it never blocks outright, only prompts. |
