...
| 2 | [../2-roadmap/Roadmap.md](../2-roadmap/Roadmap.md) | What 0-100% means, what "done" looks like |
...
| 6 | [../6-decisions/Decisions.md](../6-decisions/Decisions.md) | Why a decision was made, and what it replaced |

Plus four non-tier folders: [`plans/`](../plans) (live intent until executed),
[`archive/`](../archive) (inert docs, moved not deleted), [`generated/`](../generated)
(tool-produced output, regenerated not hand-edited), and [`plain/`](../plain) (plain-English
copies of eligible docs, written by the `house-rules:plain-docs` skill and checked by
`plain_docs_check.py`; mirrors the layout of the doc each copy is drawn from). There's also the
short [`docs/README.md`](../README.md) at the top of `docs/`, a plain-English page for people
...
| System | Owns |
|---|---|
| [hook-engine](../4-systems/hook-engine.md) | Dispatching every `house-rules` hook event to the handler that enforces it |
| [verify-suites](../4-systems/verify-suites.md) | Proving the three plugins' hook payloads produce the decisions the docs claim |
| [plugin-distribution](../4-systems/plugin-distribution.md) | Getting the plugins onto a machine and upgrading them |
| [offshoot-plugins](../4-systems/offshoot-plugins.md) | `prompt-workshop` and `agent-router` |
...
- [`architecture-backlog.md`](../architecture-backlog.md) / [`rules-backlog.md`](../rules-backlog.md) —
  ...
  [`ProjectState.md`](../3-state/ProjectState.md#cross-cutting-issues-that-belong-to-no-milestone).
...
- [`sessions/`](../sessions) — per-session audit ledgers from `tools/session_ledger.py`. Sparse —
  see [`ProjectState.md`](../3-state/ProjectState.md#cross-cutting-issues-that-belong-to-no-milestone).
