...
## 7. Only `harvest` traces its decisions; the other handlers stay quiet on success

**Status:** open, `strong`. Raised 2026-09-09.

**The friction.** The "nothing fails silently" rule (added with the comment-harvest work) says
the default output must answer *did this run, on what, and what did it decide*. `harvest`
implements that: a one-line trace on every source-file write, whether or not it fires, naming
what it measured. Every other handler still says nothing on the success path, so "the hook ran
and decided not to fire" and "the hook never ran" remain indistinguishable for seven of eight
events — which is the exact defect the rule names.

**Evidence.** `guard` returning "allow" is byte-identical to `guard` never being invoked: both
are empty stdout, exit 0. Debugging why a tripwire did not prompt means reading `hook.py`, which
is what `CLAUDE.md` says the suite's output exists to avoid. The same holds for `artifact`,
`runnable` and `standards` — `verify.py:1131-1242` has to assert on emitted *prose* precisely
because a decision has no other visible form.

**What would change.** Lift `harvest`'s three-tier shape (out of jurisdiction → silent;
in jurisdiction → one line always; `HOUSE_RULES_DEBUG=1` → verbose) into the dispatch seam, so
a handler returns a decision and the seam renders the trace. That is the same seam entry 2
proposes for the failure-mode contract, and the two should probably land together.

**Deliberately deferred.** Doing it inside the harvest change would have put every hook's output
shape into one diff, and the trace's per-write cost has only been measured for one handler.

**Open questions.**

- Does a trace on `scope` (every prompt) or `guard` (every shell command) cost more than it is
  worth? `harvest` fires on source writes only, which is a much cooler path.
- `inject` already emits `suppressOutput`. Whether a trace should respect that, or whether
  `suppressOutput` is about the reminder rather than the diagnostic, is undecided.
