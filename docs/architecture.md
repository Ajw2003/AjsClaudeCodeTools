...
`claude -p` (docs/plans/2026-09-22-rules-that-actually-load.md; docs/6-decisions/Decisions.md, 2026-09-23,
...
| `SessionStart` | `docstiers` | every session | Fourth entry on the same event, its own hook entry so a failure here can never affect `inject`/`profile`/`standards`. Checks the project root (`CLAUDE_PROJECT_DIR`, else `cwd`) for the six documentation tiers named in `skills/project-docs/SKILL.md` — `docs/1-landing/README.md`, `docs/2-roadmap/Roadmap.md`, `docs/3-state/ProjectState.md`, at least one file under `docs/4-systems/`, `docs/5-today/Today.md`, `docs/6-decisions/Decisions.md`. All present: emits nothing at all — the one other deliberate silent exception besides `handover`, since this runs every session and a trace would cost something on every one of them for what is usually true. Any missing: `additionalContext` names exactly which tiers are missing and instructs loading `house-rules:project-docs` and scaffolding them before any other work, in every repo, including one that is not a git repository at all. In a git repository, also reads the remote's owner from `.git/config` (no subprocess — handles `https://` and `git@host:` URL forms, and a worktree's `.git` file redirect) against `HOUSE_RULES_GITHUB_OWNER` (default `Ajw2003`, case-insensitive); not owned, or the remote can't be read at all, adds one more instruction: add every scaffolded path to `.git/info/exclude`, so the new docs never leave the machine or enter that repo's history. Fails loud via `systemMessage`, like `inject`. |
| `SessionStart` | `versioncheck` | every session | Fifth entry on the same event. Compares three copies of the plugin version — installed, the local marketplace clone, and GitHub's default branch — because the marketplace clone can itself go stale independently of the installed copy (`marketplace add` does not re-fetch a marketplace the device has already seen). Since 2.37.0 it also reads `~/.claude/plugins/installed_plugins.json` — what the *next* session loads, as opposed to the copy running now — so a session that merely predates an already-installed update gets a one-line "start a new session" notice instead of an update request. A real mismatch is fixed by the hook itself: it runs `claude plugin marketplace update` (only when the marketplace clone is stale) and `claude plugin update`, inside a 60-second budget (the hook's timeout is 90), then re-reads `installed_plugins.json` and counts the update as done only if the new version is there — exit 0 alone is not trusted. Success is a one-line notice to the user and the model; the running session keeps the old code until restarted. Only if the update fails does the loud `additionalContext` banner fire, naming the failure and the exact commands and telling Claude to run them itself straight away, without asking in chat — the `guard` prompt on that first command is the user's yes — falling back to the step-card format, marked `UNTESTED:`, and a stop-and-wait only if the user declines or the session has no shell tool. `HOUSE_RULES_AUTO_UPDATE=off` keeps the check but skips the automatic update. On failure it also writes a session-keyed marker file so `guard` can also surface it as a real permission prompt on the session's first `Bash`/`PowerShell` call — the one deliberate, scoped exception to "no hook keeps state between invocations" (see below). `HOUSE_RULES_VERSION_CHECK=off` disables the whole check; `HOUSE_RULES_VC_GITHUB_URL` points a fork at its own repo instead of `Ajw2003/AjsClaudeCodeTools`. GitHub's copy is fetched from `raw.githubusercontent.com` first and, if that fails, from the `api.github.com` contents endpoint derived from the same owner/repo/path (`HOUSE_RULES_VC_GITHUB_API_URL` overrides it), both inside one 4-second budget. Network failures are best-effort and never treated as "out of date" — only an actual version mismatch is — but since 2.35.0 a check that could not be completed is said to the model too: an `additionalContext` notice naming each failed route and the installed version, telling Claude to mention it in its first reply, with no banner and no marker. A check where all three copies agree stays a `systemMessage` trace only. |
| `UserPromptSubmit` | `scope` | every prompt | Restates a short reminder so the rules stay live 200 messages into a long session, after the SessionStart copy has faded from attention. Emits a short pointer form by default and the full form only when the prompt looks command/file/build-shaped, gated statelessly on the payload's `prompt` field — every failure path falls back to the short form and exits 0, since a non-zero exit here erases the user's prompt. Both forms carry a docs-tier line and an evidence line (not a step-card line — that enforcement now lives at the point it actually matters, `handover`'s own fence and evidence checks) and stay within +10% of their 2026-09-23 baseline size (long 909 chars, short 260). |
| `PreToolUse` | `guard` | `Bash`/`PowerShell` calls | Extracts the `command` field and textually matches it against rule patterns (hidden/background work, git history/index/remote writes, destructive deletes and discards). A match returns `permissionDecision: "ask"` — it never blocks outright, only prompts. **Branch-aware since 2.16.0:** a plain `commit` or `push` stops prompting when `.git/HEAD` says the checkout is on a `claude/…` branch, because the commit rule already allows those there. Nothing else is exempt — force-push, `reset`, `revert`, `clean`, `rebase`, `merge`, `cherry-pick`, `am` and `apply` prompt on every branch, and so does any command carrying `-C`/`--git-dir`/`--work-tree`, which names a repo other than the one the branch was read from. Every uncertainty (their branch, a detached HEAD, an unreadable or absent repo) lands on the prompting side and the prompt says which. Also consumes `versioncheck`'s session-keyed out-of-date marker if one is waiting: the first `Bash`/`PowerShell` call of an out-of-date session prompts once for that reason alone, even if the command itself trips no other rule, then the marker is deleted so later calls don't repeat it. **Commit-time docs check:** on a `git commit` (reusing `GUARD_R3`'s own commit pattern) it computes what the commit will ACTUALLY include, not just what is staged right now — `guard` fires before the command it judges runs, so a plain `git diff --cached --name-only` missed the common `git add f.py && git commit -m x` and `git commit -am x` forms entirely (`doc-ref c79f docs/6-decisions/Decisions.md`). `-a`/`--all`/`-am`/`-ma` on the commit statement unions in `git diff HEAD --name-only` (every tracked, modified/deleted path); an earlier `git add <paths>` in the same command (split on `&&`/`||`/`;`/newlines) adds those literal paths, resolved against `git status --porcelain -uall` by prefix match; `git add .`/`-A`/`--all` unions in every path from that listing, `-u`/`--update` only the tracked ones — all under one shared 2-second deadline, the one place `guard` shells out, never inside `branch_ownership()` itself (`verify.py` still asserts that function alone stays subprocess-free). A resulting source file (the `harvest` extension list) with nothing under `docs/` gets a reminder naming the tier to update: on a `claude/…` branch the commit still allows, gaining an `additionalContext` reminder (a `PreToolUse` `allow` decision's `additionalContext` DOES reach the model in-turn — probed, `doc-ref 8713 docs/6-decisions/Decisions.md`); anywhere else the same line is folded into the prompt `guard` already shows. A command naming another repo, a missing `git`, a timeout or any other failure never changes `guard`'s own decision — it only adds "could not tell" to whichever message was already going out. |
...
| `Stop` | `handover` | a turn about to end **whose reply hands over a command and is not already in card shape, OR claims success with no evidence behind it** | Reads `last_assistant_message` — the documented field for the just-written reply. **Card check:** stays silent when there is no fence whose info-string is a recognized shell (`bash`/`sh`/`zsh`/`shell`/`console`/`powershell`/`pwsh`/`ps1`/`cmd`/`bat`/`fish`) — a fence in another language, or with no label, is not a command handover. It also stays silent when the reply already carries the card markers (`---`, `###`, `You should see:`), because firing on a compliant reply cannot end quietly — the turn continues, `suppressOutput` has no effect, and the only thing left to say is that nothing needed saying, which is exactly the *a card never announces its own compliance* rule being broken by the hook that enforces it. A payload with no `last_assistant_message` (an older CLI) still fires, so a version difference cannot silently disable it. **Evidence check**, independent of the card check: a reply matching a success word (`works`/`working`/`fixed`/`passes`/`passing`/`passed`/`verified`/`tested`/`confirmed`/`succeeded`, word-boundary so "untested"/"unverified" never match, plus an explicit negation-window check) fires when the transcript shows no `tool_use` since the last **genuine user message** (`doc-ref 25b2 docs/6-decisions/Decisions.md`) and the reply quotes no evidence (any fenced block, or a line shaped like real output — `RESULT: PASS`/`FAIL`, an exit code, a test runner's "N passed" line). The two checks share exactly one `additionalContext` emission when both fire — two `emit()` calls would be two concatenated JSON objects, not valid hook output — though a shell-fenced reply always satisfies the evidence exemption, so in practice they never both fire live on the same reply (`doc-ref 6534 docs/6-decisions/Decisions.md`). A transcript that cannot be read, or carries no genuine user message, fails open with a `systemMessage` naming what it could not tell, never a block. Stands down on the retry (`stop_hook_active`), on `HOUSE_RULES_HANDOVER=off`, and on any other failure — it fails **open**, since a non-zero exit here would stop the turn ending at all. Never `decision: "block"`. |
...
| `PostToolUse` | `audit` | `Agent`/`Task` calls whose `tool_response.status` is `completed` | The other half of `verdict`'s audit summary: a probe showed `SubagentStop`'s `additionalContext`/`systemMessage` do **not** reach the parent session's model context in-turn, but a `PostToolUse` hook matched on `Agent\|Task` does, for a **foreground** call (`doc-ref 8313 docs/6-decisions/Decisions.md`). Reads `tool_response.agentId`/`agentType` (its own camelCase field names, a different shape from `SubagentStart`/`SubagentStop`'s flat fields), locates the transcript the same way `verdict` does, and emits the same audit summary — commands with exit status, files written/edited, tool counts, the reconcile instruction — as `additionalContext` straight to the parent model. A `status` of `async_launched` (a backgrounded call, still running) is silent: nothing has happened yet, so there is nothing to audit; `userpromptaudit` covers that call's later hand-back. |
| `UserPromptSubmit` | `userpromptaudit` | every prompt | Its own entry, separate from `scope`: covers the case `audit` cannot — a backgrounded (`run_in_background: true`) `Agent`/`Task` call whose completion arrives later as a fresh user turn carrying a `<task-notification>` (probed live; not the documented reference). Fires only when the prompt matches that shape and its `<status>` is `completed`, extracts `<task-id>`, and emits the same audit summary as `additionalContext`. Every other prompt, and every failure, is silent (or, on an actual error, a `systemMessage`, never `additionalContext`) and exits 0 — same "never erase the user's prompt" contract as `scope`. |
...
`Write` — so an `Edit` never gets the exemption. See `docs/6-decisions/Decisions.md`, "Fix the harvest
...
GitHub's copy has two routes, tried in order inside one `_GITHUB_FETCH_TIMEOUT` (4 s) budget:
`raw.githubusercontent.com`, then the contents API
(`api.github.com/repos/<owner>/<repo>/contents/<path>?ref=<ref>` with
`Accept: application/vnd.github.raw`). The API URL is derived from the raw URL, so a fork that
sets `HOUSE_RULES_VC_GITHUB_URL` gets the fallback for free; `HOUSE_RULES_VC_GITHUB_API_URL`
overrides it, and is also the seam `verify.py` uses (with `file://` URLs, so the tests never
touch the network). The second route exists because a real sandbox reset every connection to
`raw.githubusercontent.com` while `api.github.com` worked, and one blocked host blinded the check.

When the check cannot finish — both routes fail, or the marketplace clone cannot be read — and
nothing it *could* compare disagrees, it is still not "out of date": no banner, no marker for
`guard`. But it is no longer silent to the model. It emits `additionalContext` naming each failed
route and the installed version and telling Claude to mention it in its first reply, alongside
the `systemMessage` trace the user sees. Before 2.35.0 only the `systemMessage` went out, which
never reaches the model, so an unverified result looked identical to a verified one — see
`docs/6-decisions/Decisions.md`, 2026-09-24.

### versioncheck installs the update itself, and checks the install on disk

The running copy and the installed copy are different things. A plugin loads when Claude Code
starts, and `/clear` re-runs `SessionStart` from the same loaded copy. So once anything installs
a newer version — the user, a setup script, Claude Code's own background auto-update — every
session start in that process still reports the old version. Comparing only that running copy is
what made the check ask for an update that was already installed; the fix there is a restart. So
`installed_plugins.json` is read too, and "right version on disk, old version running" gets the
restart notice, not a banner or a marker.

When an update really is needed, the hook runs it, not the user or Claude. Neither `run.sh` nor
`guard` is on that path: it is one `SessionStart` handler, running only when versions disagree,
with its own timeout. The `claude` it runs is whatever is on `PATH`; `HOUSE_RULES_VC_CLAUDE` (a
JSON argv list) replaces it, which is how `verify.py` swaps in a fake that never touches the
real install. Cloud environments whose setup script only runs when the environment cache is
built are covered by the same path: the hook runs on every session start, cached or not. See
`docs/6-decisions/Decisions.md`, 2026-09-26.
